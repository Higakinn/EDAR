...
describe('RestaurantListコンポーネント', () => {
    ...
    test('お店の情報が取得できなければエラー表示', () => {
        ...
        // == 検証 ==
        /** レンダリングされたテキストが'お店の情報を取得できませんでした。'であればOK */
        expect(screen.getByText('お店の情報を取得できませんでした。')).toBeInTheDocument();
    });
    ...
    test('位置情報の利用が許可されていなければエラー表示', () => {
        ...
        render(<RestaurantList
            ...
            expanded={[]}
        />);
        expect(screen.getByText('位置情報の利用が許可されていません')).toBeInTheDocument();
    });
    ...
    test('選択ジャンルのお店が周辺に無ければ、その旨を表示', () => {
        ...
        render(<RestaurantList
            ...
            expanded={[]}
        />);
        expect(screen.getByText('近くに該当ジャンルのお店がありませんでした。')).toBeInTheDocument();
    });
