...
export const SelectGenre = withRouter((props) => {
  ...
  // ジャンルが変更された際の処理
  function changedgenre(
    event: React.ChangeEvent<{ name?: string | undefined; value: any | string }>
  ) {
  ...
  // お店の情報を取得し終えたら画面遷移
  useEffectCustom(() => {
    props.history.push(`/range=${range.label}/genre=${genre}`);
  }, [shops]);
        ...
        <Grid container spacing={3} alignItems="center" justify="center">
          ...
          <Grid item xs="auto">
            <FormControl className={classes.formControl} variant="outlined">
              {/* TODO: (警告が出る) */}
              <InputLabel htmlFor="select" color="primary" id="label">
                ジャンル
              </InputLabel>
              <Select
                data-testid="select"
              ...
              >
                {genres.map((output: Genre, index: number) => (
                  <MenuItem key={index} value={output.code}>
                    {' '}
                    {output.name}{' '}
                  </MenuItem>
                ))}
