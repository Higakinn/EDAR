...
export async function getShopList(url: string): Promise<Shop[]> {
  const shops: Shop[] = [
    ...
    {
      access:
        'JR新宿駅西口より徒歩15分/地下鉄大江戸線都庁前駅A5番出口より徒歩4分/京王新線初台駅より徒歩10分',
      ...
      close:
        '※ランチではお席の指定はお断りさせて頂いております。※ご予約時間から15分経過した場合場合はキャンセルとさせて頂きます。',
      coupon_urls: {
        pc: 'https://www.hotpepper.jp/strJ001201792/map/?vos=nhppalsa000016',
        sp:
          'https://www.hotpepper.jp/strJ001201792/scoupon/?vos=nhppalsa000016',
      },
      course: 'あり',
      english: 'あり',
      ...
      lng: 139.6885958675,
      logo_image:
        'https://imgfp.hotp.jp/IMGH/74/75/P030557475/P030557475_69.jpg',
      lunch: 'あり',
      middle_area: {
        code: 'Y055',
        name: '新宿',
      ...
      non_smoking: '全面禁煙',
      open:
        '月～日、祝日、祝前日: 08:00～22:00 （料理L.O. 21:00 ドリンクL.O. 21:00）',
...
export async function getPosition(): Promise<{
  ...
  return new Promise((resolve, reject) => {
    process.nextTick(() => resolve(position));
  });
}
